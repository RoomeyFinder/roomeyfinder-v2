-- Use the actual listed home for homeowner distance calculations. A seeker's
-- private profile location remains their preferred search area.
...
as $$
  with requester as (
    select
      ...
      coalesce(
        (
          select home_address.location
          from public.homes as home
          join public.home_addresses as home_address on home_address.home_id = home.id
          where home.owner_id = profile.id
            and home.status = 'active'::public.home_status
          order by home.created_at desc
          limit 1
        ),
  ...
  compatible_pool as (
    select
      ...
      coalesce(candidate_home_address.location, candidate_private.location) as candidate_location,
    ...
    left join public.home_addresses as candidate_home_address
      on candidate_home_address.home_id = candidate_home.id
