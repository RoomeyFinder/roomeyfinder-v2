...
export function FlowProgress({ current, gates }: { current: string; gates: readonly { id: string; label: string; icon: LucideIcon }[] }) {
    ...
    <div className="rounded-brand-md border bg-card p-3 shadow-sm md:p-4">
      <div className="grid grid-cols-4 gap-2">
        {gates.map((gate, index) => {
          ...
          return (
            <div key={gate.id} className="relative flex items-center justify-center space-x-2 md:justify-start">
              <div className={`z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border text-xs ${complete || active ? "border-brand bg-brand text-white" : "border-border bg-muted text-muted-foreground"}`}>
                {complete ? <Check className="h-4 w-4" /> : <Icon className="h-4 w-4" />}
              </div>
              <span className={`relative z-10 hidden bg-card pr-1.5 text-sm font-medium sm:block ${active ? "text-foreground" : "text-muted-foreground"}`}>
                {gate.label}
              </span>
...
export function ChoiceCard({ selected, icon: Icon, title, description, badge, onClick }: { selected: boolean; icon: LucideIcon; title: string; description: string; badge: string; onClick: () => void }) {
  return (
    <button type="button" aria-pressed={selected} onClick={onClick} className={`rounded-brand-md border p-6 text-left transition-all hover:-translate-y-0.5 hover:border-brand/50 hover:shadow-md ${selected ? "border-brand bg-secondary/50 shadow-md" : "bg-card"}`}>
      <div className="flex items-start justify-between gap-4"><div className="flex h-11 w-11 items-center justify-center rounded-brand-md bg-secondary text-brand"><Icon className="h-5 w-5" /></div>{selected ? <CheckCircle2 className="h-5 w-5 text-brand" /> : null}</div>
