        ...
        {gates.map((gate, index) => {
          ...
          return (
            ...
            >
              ...
              >
                <span
                  className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border text-xs ${complete || active ? "border-brand bg-brand text-white" : "border-border bg-muted text-muted-foreground"}`}
                >
                  {complete && !active ? (
                ...
                <span
                  className={`hidden pr-1.5 text-sm font-medium sm:block ${active ? "text-foreground" : canNavigate ? "text-muted-foreground hover:text-foreground" : "text-muted-foreground"}`}
                >
                  {gate.label}
                </span>
              </button>
              {index < gates.length - 1 ? (
                <div className="absolute right-[-1rem] top-4 z-0 hidden h-px w-[calc(100%-4rem)] bg-border md:block" />
              ) : null}
            </div>
          );
        })}
...
export function StepIntro({
  ...
  return (
    <div className="mb-7 flex gap-4">
      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-brand-md bg-secondary text-brand">
    ...
    <Card className="h-fit bg-secondary/30">
      <CardContent className="p-6 md:p-8">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-background text-brand">
...
export function ChoiceCard({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`rounded-brand-md border p-6 text-left transition-all hover:-translate-y-0.5 hover:border-brand/50 hover:shadow-md ${selected ? "border-brand bg-secondary/50 shadow-md" : "bg-card"}`}
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex h-11 w-11 items-center justify-center rounded-brand-md bg-secondary text-brand">
          <Icon className="h-5 w-5" />
        </div>
        <div className="flex items-center gap-2">
          {action ? (
            <span
              ...
              className="rounded-full bg-background px-2.5 py-1 text-xs font-semibold text-brand shadow-sm"
            >
              {action.label}
            </span>
          ) : null}
