...
import {
  ...
  SideNote,
  StepIntro,
} from "@/components/roommate-flow/shared";
...
export function HomeStep({
  ...
  if (editing) {
    return (
      <HomeForm
        initialDraft={homeownerDraft}
        existingPhotos={existingPhotos}
        saving={saving}
        onBack={() => setEditing(false)}
        onContinue={onSaveHome}
        onDeletePhoto={onDeleteHomePhoto}
      />
    );
  }
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <StepIntro
        ...
        description="Your answer controls who appears in discovery. You can change your plan later by updating your profile."
        icon={HomeIcon}
      />
      <div className="grid gap-4 md:grid-cols-2">
        <ChoiceCard
          ...
          onClick={() => {
            select("homeowner");
            if (!hasExistingHome) setEditing(true);
          }}
          action={
            hasExistingHome
              ? {
        ...
        <ChoiceCard
          ...
          description="Find a room that fits your needs, with the option to team up and find one together."
          onClick={() => select("seeker")}
          badge="Home seeker"
        />
      </div>
      <Card className="mt-6">
        <CardContent className="flex flex-col gap-5 p-6 md:flex-row md:items-center md:justify-between">
          <div className="flex gap-4">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-secondary text-brand">
          ...
          <button
            type="button"
            role="switch"
            aria-checked={effectiveTeamUp}
            aria-disabled={teamUpDisabled}
            disabled={teamUpDisabled}
            aria-label="Open to finding a place together"
            onClick={toggleTeamUp}
            className={`flex h-7 w-12 shrink-0 items-center rounded-full p-1 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 ${effectiveTeamUp ? "justify-end bg-brand" : "justify-start bg-muted-foreground/30"}`}
          >
      ...
      <div className="mt-6 flex flex-col items-start justify-between gap-4 border-t pt-6 sm:flex-row sm:items-center">
        ...
        {selected === "homeowner" && hasActiveHome ? (
          ...
          <Button
            disabled={selected !== "seeker" || saving}
            onClick={() => void continueAsSeeker()}
          >
            {saving ? "Saving plan..." : "See my matches"} <ArrowRight />
          </Button>
        )}
      </div>
    </div>
...
function HomeForm({
  ...
  saving: boolean;
  onBack: () => void;
  onContinue: (draft: HomeDraft) => Promise<boolean>;
  onDeletePhoto: (homeId: string, photo: HomePhotoDraft) => Promise<boolean>;
}) {
  const { draft, photos, update, selectPhotos, submit } = useHomeForm(
    onContinue,
    initialDraft,
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
      <Card>
        <CardHeader>
          <button
            type="button"
            onClick={onBack}
            className="mb-4 flex w-fit items-center gap-1 text-sm font-medium text-muted-foreground hover:text-foreground"
          >
            <ChevronLeft className="h-4 w-4" /> Back to choices
          </button>
          <StepIntro
            eyebrow="Homeowner"
            title={initialDraft ? "Edit your home details" : "Add your home details"}
            description="Complete the essentials and your home will be published so compatible seekers can discover you."
            icon={HomeIcon}
          />
        </CardHeader>
        <CardContent>
      ...
      <SideNote title="Homeowner" icon={ShieldCheck}>
        <p>
          Your listing puts you in the driver&apos;s seat. We&apos;ll only show you to people whose
          preferences fit your home.
        </p>
