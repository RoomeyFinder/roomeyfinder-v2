...
export function PreferencesStep({ initialDraft, onContinue, saving }: PreferencesStepProps) {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
      <Card><CardHeader><StepIntro eyebrow="Step 2 of 3" title="Set your non-negotiables" description="This is where we tune the search. You can always update these preferences later." icon={Settings} /></CardHeader><CardContent>
        <form className="space-y-6" onSubmit={handleSubmit}>
          {validationError ? <p role="alert" className="rounded-brand-md border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">{validationError}</p> : null}
          ...
          <div className="grid gap-4 sm:grid-cols-2"><Field label="Move in from" htmlFor="move-in-from"><Input id="move-in-from" required type="date" min={today} max={draft.moveInTo || undefined} value={draft.moveInFrom} onChange={(event) => update("moveInFrom", event.target.value)} /></Field><Field label="Move in by" htmlFor="move-in-to"><Input id="move-in-to" required type="date" min={draft.moveInFrom || today} value={draft.moveInTo} onChange={(event) => update("moveInTo", event.target.value)} /></Field></div>
